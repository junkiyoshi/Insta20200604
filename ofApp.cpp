...
void ofApp::setup() {
	...
	vector<string> file_path_list = {
		"openframeworks 2020-05-25.mp4",
		"openframeworks 2020-04-24.mp4", 
		"openframeworks 2020-05-13.mp4", 
		"openFrameworks 2020-05-21.mp4",
	};
...
void ofApp::draw() {
	...
	for (int k = 0; k < 2; k++) {
		...
		for (int m = 0; m < 2; m++) {
			...
			ofPushMatrix();
			ofTranslate(
				m * this->cap_size.width * 1.1 - this->cap_size.width * 0.55 + this->rect_size * 0.5,
				k * this->cap_size.height * 1.1 - this->cap_size.height * 0.55 - this->rect_size * 0.5,
				0);
